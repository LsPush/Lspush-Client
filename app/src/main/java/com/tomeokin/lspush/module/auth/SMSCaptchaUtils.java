...
public class SMSCaptchaUtils {
    public static Log log = AppLogger.networkLog("Mob SMS");
    ...
    public static final class SMSHandler extends Handler {
        private final SMSRequestCallback mCallback;

        public SMSHandler(@NonNull SMSRequestCallback callback) {
            mCallback = callback;
        }
        ...
        public void dispatchMessage(Message msg) {
            if (msg.arg1 != SMSSDK.RESULT_COMPLETE) {
                Throwable cause = new SMSException(msg.what, (Throwable) msg.obj);
                log.log(cause);
                mCallback.onSMSRequestFailed(cause, msg.what);
                return;
            }

            if (msg.what == GET_SUPPORTED_COUNTRIES) {
                final HashMap<String, String> countryList = getCountryList((ArrayList<HashMap<String, Object>>) msg.obj);
                mCallback.onGetCountryListResponse(countryList);
            } else if (msg.what == SEND_CAPTCHA) {
                mCallback.onSendSMSResponse((boolean) msg.obj);
                ...
                mCallback.onCheckCaptchaResponse(phone, countryCode);
            }
        }
